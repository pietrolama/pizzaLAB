...
document.addEventListener("DOMContentLoaded", () => {
    const salvaDiarioBtn = document.getElementById("salva-diario-btn");
    if (!salvaDiarioBtn) {
        console.warn("Il pulsante 'salva-diario-btn' non è stato trovato nel DOM.");
        return;
    }

    salvaDiarioBtn.addEventListener("click", async () => {
        try {
            const tipoPizza = document.getElementById("tipo_pizza")?.value;
            const metodoImpasto = document.getElementById("tipo_impasto")?.value;

            if (!tipoPizza || !metodoImpasto) {
                alert("Assicurati di aver selezionato un tipo di pizza e un metodo di impasto.");
                return;
            }
